...
namespace Photon.Chat
{
    ...
    public class ChatClient : IPhotonPeerListener
    {
        ...
        /// <summary>
        /// Applies appSettings and connects to the Photon Chat Cloud.
        /// </summary>
        public bool ConnectUsingSettings(ChatAppSettings appSettings)
        {
            ...
            this.AppId = appSettings.AppIdChat;
        ...
        private bool ConnectIntern()
        {
            ...
            bool isConnecting = this.Peer.Connect(this.NameServerAddress, this.AppId, null, proxyServerAddress: this.ProxyServerAddress);
            if (isConnecting)
            {
        ...
        private void TryAuthenticateOnNameServer()
        {
            ...
            {
                this.didAuthenticate = this.AuthenticateOnNameServer(this.AppId, this.AppVersion, this.ChatRegion, this.AuthValues);
                if (!this.didAuthenticate)
                {
                    if (this.LogLevelPeer >= LogLevel.Error)
                    {
        ...
        private bool ConnectToFrontEnd()
        {
            ...
            if (!this.Peer.Connect(this.FrontendAddress, this.AppId, this.AuthValues.Token, proxyServerAddress: this.ProxyServerAddress))
            {
                if (this.LogLevelPeer >= LogLevel.Error)
                {
